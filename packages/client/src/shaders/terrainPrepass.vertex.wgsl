...
// Courtyard pad — must match snow.vertex.wgsl exactly or the prepass would
// render against a surface that is not the one on screen.
...
fn main(input: VertexInputs) -> FragmentInputs {
    ...
    // matching snow.vertex.wgsl exactly so the prepass renders against the same
    // surface the beauty pass draws. See the note there.
